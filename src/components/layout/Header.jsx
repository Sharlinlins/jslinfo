      ...
      <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-4 flex justify-between items-center">
          ...
          >
            ...
            <span className={`font-bold text-xl tracking-tight ${scrolled ? 'text-gray-800' : 'text-gray-800'}`}>
            ...
            {navItems.map(item => (
              ...
              >
                ...
                <span className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-indigo-600 to-purple-600 transition-all duration-300 ${
                  !isServicePage && activeSection === item.id ? 'w-full' : 'w-0 group-hover:w-full'
                }`}></span>
              </button>
            ))}
            <button
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 py-4 px-5 shadow-xl animate-fade-in-up">
            {navItems.map(item => (
              <button
                key={item.id}
                onClick={() => handleNavClick(item)}
